...
module.exports = class cmd extends Command {
    ...
    async run(interaction, args) {
        if (GetResourceState("qb-weathersync") !== "started") return interaction.reply("This command requires QBCore's `qb-weathersync` to work");
        // doesn't give any feedback to rely on :/
        emit("qb-weathersync:server:setTime", args.hour, "0");
        zlog.info(`[${interaction.member.displayName}] set time to hour ${args.hour}`);
        return interaction.reply("Time has been set");
    }
};
